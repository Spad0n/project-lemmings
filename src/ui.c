...
void ui_widget_item(Texture2D texture, Rectangle rect, Rectangle source) {
    ...
    //size_t tile_width = UI_TILE_SIZE * (int)(rect.width / UI_TILE_SIZE);
    //rect.x = rect.x + (rect.width - tile_width) / 2;
    //rect.width = tile_width;
    //size_t tile_height = UI_TILE_SIZE * (int)(rect.height / UI_TILE_SIZE);
    //rect.y = rect.y + (rect.height - tile_height) / 2;
    //rect.height = tile_height;

    //rect.x = ceilf(rect.x + (rect.width - source.width) / 2);
    //rect.y = ceilf(rect.y + (rect.height - source.height) / 2);
...
static Rectangle ui_layout_slot(Layout *l, const char *file_path, int line) {
    if (l->i >= l->count) {
...
void ui_layout_stack_push(Layout **ls, Layout_Orient orient, Rectangle rect, size_t count, float gap) {
    Layout l = ui_make_layout(orient, rect, count, gap);
    array_push(*ls, l);
}
...
Rectangle layout_stack_slot_loc(Layout **ls, const char *file_path, int line) {
    assert(array_size(*ls) > 0);
    return ui_layout_slot(&array_last(*ls), file_path, line);
}
